...
public class MLP{
	...
	private static double espected_output;
	...
	public void setCiclos(int value){
		ciclos = value;
	}
	 ...
	 * @return
	 */
	public static double getNet2(int col){
		return net2[col];
	}
	 ...
	 * @param value
	 */
	public static void setNet2(int col, double value){
		net2[col] = value;
	}
	 ...
	 * @param col
	 */
	public static void initNet2(int col){
		net2 = new double[col];
	}
	...
	public void setEntradas(double[][] e){
		entradas = e;
	}
	
	public void setSaida(double[] s){
		this.saidas = s;
	}
	 ...
	 * @param col
	 */
	public static double getSaidas(int col){
		return saidas[col];
	}
	...
	public static void randomize(){
		...
		for (int j=0; j<getNumHidden(); j++) {
			setBias1(j,-1+Math.random()*((1 - (-1))+1));
			setAntigodeltabs1(j, 0.0d);
			setDeltabs1(j, 0.0d);
			
			for (int i=0; i<getNumInputs(); i++) {
				setPesos1(j, i, -1+Math.random()*((1 - (-1))+1));
		...
		for (int j=0; j<getNumOutputs(); j++) {
			setBias2(j,-0.1 +(random.nextInt((8192)/8192)));
		...
		for(int kl=0; kl<getCiclos(); kl++) {
			...
			for(int itr=0; itr<getNumTraining(); itr++) {
				...
				for(int j=0;j<getNumOutputs();j++)
				{
					...
					for(int i=0;i<getNumHidden();i++){
						setNet2(j,getNet2(j)+(getPesos2(j, i)*getFnet1(i)));
					}
				...
				for(int j=0;j<getNumOutputs();j++) {
					setErro2(itr,j, (getSaidas(j) - getNetout(itr,j)));
					//impressão dos dados de saída
					//System.out.println("Ciclo:"+ " "+ getCicloAtual() + "  "+	"Exemplo:" +" "+ (itr+1));

					//System.out.println("Saída desejada:"+" "  +  getSaidas(j)+ "  "+ "Saída calculada:" 
					//+" " +getNetout(itr,j));
...
//		System.out.println(activationFunction2());
//		
//		//-----------------------------------------
//		setSemente(1315);
//		setCiclos(1000);
...
//	}
	...
	public static double prevision() {
		...
		for(int k = 0; k < c.getTreinamentoSize(); k++){
			for (int j=0;j<getNumHidden();j++) {
				...
				for(int i=0;i<getNumInputs();i++){
					net1[j] = getNet1(j) + (getPesos1(j, i)* getEntradas(0, i));
			...
			for(int j=0;j<getNumOutputs();j++)
			{
				...
				for(int i=0;i<getNumHidden();i++){
					net2[j] = net2[j] + (getPesos2(j, i)*getFnet1(i));
				}
				eb=(double)(Math.exp((double)((-1.0d)*(getNet2(j)))));
				
				netout[k][j] = (double) (1.0/(1.0+eb));
			}
		}
	...
	public static void setOutput(double[][] netout2) {
		output = netout2;
	}

	public static double getEspected_output() {
		return espected_output;
	}
